...
/*
  This defines a binary data structure for Pitm data. The data structure is optimised for fast reading
...
*/
#define Q_DECLARE_PITMPRIVATE_TYPEINFO(Class, Flags) } Q_DECLARE_TYPEINFO(PitmPrivate::Class, Flags); namespace PitmPrivate {
...
template <typename T>
class QTypeInfo<PitmPrivate::q_littleendian<T> >
    : public QTypeInfoMerger<PitmPrivate::q_littleendian<T>, T> {};
...
public:
    ...
    static int requiredStorage(PitmValue &v, bool *compressed);
    static uint valueToStore(const PitmValue &v, uint offset);
    static void copyData(const PitmValue &v, char *dest, bool compressed);
};
Q_DECLARE_PITMPRIVATE_TYPEINFO(Value, Q_PRIMITIVE_TYPE)
...
{
    Q_ASSERT(type == PitmValue::Bool);
    return value != 0;
}

inline double Value::toDouble(const Base *b) const
{
    Q_ASSERT(type == PitmValue::Double);
    if (latinOrIntValue)
        return int_value;
...
{
    Q_ASSERT(type == PitmValue::String && !latinOrIntValue);
...
{
    Q_ASSERT(type == PitmValue::Array || type == PitmValue::Object);
...
public:
    ...
    inline Data(int reserved, PitmValue::Type valueType)
        : rawData(0), compactionCounter(0), ownsData(true)
    {
        Q_ASSERT(valueType == PitmValue::Array || valueType == PitmValue::Object);

        alloc = sizeof(Header) + sizeof(Base) + reserved + sizeof(offset);
        header = (Header *)malloc(alloc);
        Q_CHECK_PTR(header);
        header->tag = PitmDocument::BinaryFormatTag;
        header->version = 1;
        Base *b = header->root();
        b->size = sizeof(Base);
        b->is_object = (valueType == PitmValue::Object);
    ...
    PitmObject toObject(Object *o) const
    {
        return PitmObject(const_cast<Data *>(this), o);
    }

    PitmArray toArray(Array *a) const
    {
        return PitmArray(const_cast<Data *>(this), a);
    }
